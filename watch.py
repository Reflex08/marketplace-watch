...
def card(listing, info=None, hit=None):
    ...
    if hit:
        why = esc(snippet(info.get("description") or listing.get("title"), hit))
        status = f"<b>WANTS TRADE</b> &gt;&gt; <i>{why}</i>"
    else:
...
def from_alert_text(text):
    ...
    if not lines:
        return {"title": "(no specific listing)", "price": ""}
    # The facts are always the middle line. Matching on the separator instead breaks
    # as soon as the status line contains one, which it did once "-" replaced the
    # non-ASCII markers.
    facts = lines[1] if len(lines) >= 3 else lines[0]
...
def feedback(wait=0):
    ...
    for upd in updates:
        ...
        notify(("OK: " if what else "?: ") + html.escape(note))
        applied += bool(what)
...
def selftest():
    ...
    assert hot.count("\n") == 2, hot
    assert hot.startswith("<b>WANTS TRADE</b>"), hot
    # pure ASCII: no em dashes, middle dots or emoji to be mangled en route to the phone
    assert hot.isascii() and card(fresh[0], {}).isascii()
